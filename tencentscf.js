...
/*  console.log(`更新环境变量`)
  ...
  .finally(async () => {
    ...
    fs.writeFile('action.js', `var action = `+process.env.action+`;action > 0 ? require("./sendNotify").sendNotify("云函数部署异常！请重试","点击通知，登入后查看详情",{ url: process.env.GIT_HUB_SERVER_URL + "/" + process.env.GIT_HUB_REPOSITORY + "/actions/runs/" + process.env.GIT_HUB_RUN_ID + "?check_suite_focus=true" }): ""`,'utf8',function(error){
        if(error){
            console.log(error);
            return false;
        }
